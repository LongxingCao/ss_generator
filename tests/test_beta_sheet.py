...
def test_build_beta_sheet():
    ...
    res_list = ssg.beta_sheet.build_ideal_flat_beta_sheet('antiparallel', 10, 5)
    ...
    bp_map = {i : len(res_list[0]) - i for i in range(len(res_list[0]))}
    for i in range(1, len(res_list)):
        res_list[i] = ssg.beta_sheet.attach_beta_strand_to_reference(res_list[i], res_list[i - 1], 'antiparallel', bp_map, i % 2)
