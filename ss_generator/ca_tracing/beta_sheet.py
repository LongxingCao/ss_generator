...
def superimpose_sheet(sheet1, sheet2):
    '''Superimpose sheet1 to sheet2. Return the new coordinates
    of sheet1.
    '''
    P1 = [p for strand in sheet1 for p in strand]
    P2 = [p for strand in sheet2 for p in strand]

    M, t = geometry.get_superimpose_transformation(P1, P2)
   
    new_sheet = []
    for strand in sheet1:
        new_sheet.append([np.dot(M, p) + t for p in strand])
